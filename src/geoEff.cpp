...
// getHadronContainmentThrows for FD GEC outside veto region
std::vector< std::vector< std::vector< uint64_t > > > geoEff::getHadronContainmentThrows_FD_GEC_outsideVeto(bool ignore_uncontained){
  ...
  if (ignore_uncontained) {
    int origContained = 0;
    std::vector< std::vector< bool > > vecOrigContained = getHadronContainmentOrigin_FD_GEC_outsideVeto();
    for (unsigned int i = 0; i < vetoSize.size(); i++){
      for (unsigned int j = 0; j < vetoEnergy.size(); j++){
	if (vecOrigContained[i][j]) origContained++;
      }
  ...
  for (unsigned int t = 0; t < N_THROWS; t++){
    ...
    for (unsigned int i = 0; i < vetoSize.size(); i++){
      for (unsigned int j = 0; j < vetoEnergy.size(); j++){
        // Check containment and set bit
        if (isContained_FD_GEC_outsideVeto(transformedEdeps, hitSegEdeps, vetoSize[i], vetoEnergy[j]))
        {
	         hadronContainment[i][j][t/64] |= ((uint64_t)1)<<(t%64);
	      }
      }
...
// getHadronContainmentOrigin for FD GEC (outside veto region)
std::vector< std::vector< bool > > geoEff::getHadronContainmentOrigin_FD_GEC_outsideVeto(){
  // Initialize return vector
  std::vector< std::vector< bool > > hadronContainment(vetoSize.size(), std::vector< bool >(vetoEnergy.size(), false));
  ...
  for (unsigned int i = 0; i < vetoSize.size(); i++){
    for (unsigned int j = 0; j < vetoEnergy.size(); j++){
      if (isContained_FD_GEC_outsideVeto(hitSegPosOrig, hitSegEdeps, vetoSize[i], vetoEnergy[j])) hadronContainment[i][j] = true;
    }
  }
...
// isContainde for FD GEC, count all energy outside Veto
bool geoEff::isContained_FD_GEC_outsideVeto( Eigen::Matrix3Xf hitSegments, std::vector<float> energyDeposits, float vSize, float vetoEnergyThreshold ){
  ...
  for (unsigned int i = 0; i < energyDeposits.size(); i++){
    for (int dim = 0; dim < 3; dim++){
      // low
      if ( hitSegments(dim, i)-OffAxisOffset[dim] < active[dim][0]+vSize ) {
        vetoEnergy += energyDeposits[i];
        break; // Only count each energy deposit once
      }
      // high
      if ( hitSegments(dim, i)-OffAxisOffset[dim] > active[dim][1]-vSize ) {
        vetoEnergy += energyDeposits[i];
        break; // Only count each energy deposit once
      }
    }
